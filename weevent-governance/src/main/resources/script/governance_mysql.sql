...
create TABLE t_topic_event_count (
  `id` INT(11) NOT NULL AUTO_INCREMENT  COMMENT '主键id',
  `create_date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建日期',
  `last_update` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON update CURRENT_TIMESTAMP COMMENT '修改日期',
  `brokerId` INT(11) NOT NULL COMMENT 'broker主键id',
  `eventCount` INT(11)  NOT NULL COMMENT '事件统计数',
  `groupId` VARCHAR(64)  NOT NULL COMMENT '群组id',
  `topicName` VARCHAR(128)  NOT NULL COMMENT '主题名称',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='主题事件数量统计表';
